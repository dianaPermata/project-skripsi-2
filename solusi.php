...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add'])) {
        ...
        $solusi = $_POST['solusi'];
        $sql = "INSERT INTO solusi (kode, id_penyakit, solusi) VALUES ('$kode', '$id_penyakit', '$solusi')";
        ...
        $id_penyakit = $_POST['id_penyakit'];
        $solusi = $_POST['solusi'];
        $sql = "UPDATE solusi SET kode='$kode', id_penyakit='$id_penyakit', solusi='$solusi' WHERE id=$id";
        mysqli_query($conn, $sql);
...
$solusi_query = mysqli_query($conn, "SELECT solusi.*, penyakit.nama AS nama_penyakit 
                                     FROM solusi 
                                     INNER JOIN penyakit ON solusi.id_penyakit = penyakit.id");

$penyakit = mysqli_query($conn, "SELECT * FROM penyakit");

// Handle pagination
$total_records = mysqli_num_rows($solusi_query);
$total_pages = ceil($total_records / $limit);

// Fetch data with pagination
$solusi = mysqli_query($conn, "SELECT solusi.*, penyakit.nama AS nama_penyakit 
                                     FROM solusi 
                                     INNER JOIN penyakit ON solusi.id_penyakit = penyakit.id 
...
<head>
    <meta charset="utf-8" />
    <title>CekMental - Data Solusi</title>
...
<body>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Tambah Data Solusi</h5>
                            ...
                            <div class="modal-body">
                                <form method="POST" action="solusi.php">
                                    ...
                                    <div class="mb-3">
                                        <label for="id_penyakit" class="col-form-label">Penyakit:</label>
                                        <select id="id_penyakit" name="id_penyakit" class="form-select">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="POST" action="solusi.php">
                    ...
                    <div class="mb-3">
                        <label for="edit-id_penyakit" class="col-form-label">Penyakit:</label>
                        <select id="edit-id_penyakit" name="id_penyakit" class="form-select">
                            <?php mysqli_data_seek($penyakit, 0); // Reset pointer to beginning ?>
                            <?php while ($row = mysqli_fetch_assoc($penyakit)) : ?>
                                <option value="<?php echo $row['id']; ?>"><?php echo $row['kode'] . " | " . $row['nama']; ?></option>
                            <?php endwhile; ?>
                        </select>
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                ...
                                <th width="10" class="text-center">Penyakit</th>
                                <th width="50" class="justify">Solusi</th>
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td width="10" class="text-center"><?php echo $row['nama_penyakit']; ?></td>
                                    <td width="50" class="justify"><?php echo $row['solusi']; ?></td>
                                    <td width="10" class="text-center">
                                    <a href="#" class="btn btn-warning m-1" data-bs-toggle="modal" data-bs-target="#editModal" onclick="fillEditModal(<?php echo $row['id']; ?>, '<?php echo $row['kode']; ?>', '<?php echo $row['id_penyakit']; ?>', '<?php echo $row['solusi']; ?>')">Edit</a>
                    ...
                    <ul class="pagination">
                        <?php for ($i = 1; $i <= $total_pages; $i++) : ?>
                            <li class="page-item <?php echo ($page == $i) ? 'active' : ''; ?>">
                                <a class="page-link" href="solusi.php?page=<?php echo $i; ?>"><?php echo $i; ?></a>
                            </li>
                        <?php endfor; ?>
                    </ul>
                </nav>
    ...
    <script>
        function showAddModal() {
            ...
            document.getElementById('id_penyakit').value = '';
            document.getElementById('solusi').value = '';
        }

        function fillEditModal(id, kode, id_penyakit, solusi) {
            document.getElementById('edit-id').value = id;
    document.getElementById('edit-kode').value = kode;
    document.getElementById('edit-id_penyakit').value = id_penyakit;
